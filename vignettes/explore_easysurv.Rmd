...
We're going to use `easy_adtte` as an example data set. Since it's a package that comes loaded with `easysurv`, we don't need any of the above functions.
...
This function uses `plot_km()` to generate the KM curves. You can also use `plot_km()` directly, or pass additional arguments to `get_km()`, to customize the plot.

For example, by default, shapes are used in place of group names in the risk table beneath the plot to save space. You can change this by setting `risktable_circles = FALSE`.
...
`purrr::possibly()` is leveraged to help code can run smoothly even if the model fitting process fails. This is particularly useful when testing multiple distributions, as the best distribution is not known *a priori*.
...
If you encounter any bugs or issues, please reach out to the authors.
