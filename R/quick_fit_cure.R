#' Get information and plots for flexsurvcure mixture cure models
#'
#' This function checks whether standard parametric survival models converge,
#' by fitting the models via \code{"flexsurvcure"}, and for those
...
#' @export
#'
#' @return An object of class \code{quick_fit_cure} containing the following
#' elements:
...
#' each distribution}
#' \item{cure_fractions}{The cure fractions for each distribution, assuming a
#' logistic link.}
#' \item{predicted_fits}{A tibble showing predicted survival proportions over
...
quick_fit_cure <- function(data,
  ...
  quick_fit_select(fit_type = "cure",
                   data = data,
                   time = time,
                   event = event,
                   strata = strata,
