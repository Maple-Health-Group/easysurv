...
`easysurv` comes pre-loaded with a few data sets borrowed from other packages. One of these is the `lung` data set from the `survival` package, which is accessible by calling `ms_lung`. We'll store it into `surv_data` and inspect the first few rows of the data.
...
```{r quick-km, warning=FALSE, message=FALSE, fig.width=6, fig.height=5}
